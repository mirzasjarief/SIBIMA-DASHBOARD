...
df_download_revenue = revenue[[
    'transaction_number', 'transaction_date', 'due_date', 'status_description', 'description', 'customer_name', 'product_id', 'item_name', 'quantity', 
    'unit' ,'price', 'cogs', 'transaction_total_item', 'do_transaction_number', 'so_transaction_number']].copy()

df_download_revenue.columns = [
    'No. Transaksi SI', 'Tanggal Transaksi', 'Tanggal Jatuh Tempo', 'Status', 'Deskripsi', 'Nama Kostumer', 'ID Produk', 'Nama Barang', 'Qty SI', 
    'Satuan', 'Harga Jual', 'HPP', 'Total Transaksi', 'No. Transaksi DO', 'No. Transaksi SO'
]
...
# FIX DO: Mapping dari GRN menggunakan detail_id (GRN) ke grn_detail_id (DO)
grn_map = df_grn_f[['detail_id', 'product_id', 'so_detail_id']].drop_duplicates()
df_do_f = pd.merge(
    df_do_f, 
    grn_map, 
    left_on=['grn_detail_id', 'product_id'], 
    right_on=['detail_id', 'product_id'], 
    how='left',
    suffixes=('', '_map')
)
df_do_f['so_detail_id'] = df_do_f['so_detail_id'].fillna(0)
...
# FIX SI: Mapping dari DO menggunakan detail_id (DO) ke do_detail_id (SI)
df_si_f = pd.merge(
    df_si_f, 
    do_map, 
...
nopr_to_exclude = ['PR-25120251', 'PR-25120219', 'PR-25120177', 'PR-25120212']
...
# ---------------------------------------------------------
reconcile_master['qty_so'] = reconcile_master['qty_so'].fillna(0)
reconcile_master['qty_grn_base'] = reconcile_master['qty_grn_base'].fillna(0)
reconcile_master['qty_do_comp'] = reconcile_master['qty_do_comp'].fillna(0)
qty_grn_adjusted = reconcile_master[['qty_grn_base', 'qty_so']].min(axis=1)

# 2. Hitung Adjusted GRN (Plafon adalah SO)
# Jika SO 10, GRN 10 -> min(10, 10) = 10
qty_grn_adjusted = reconcile_master[['qty_grn_base', 'qty_so']].min(axis=1)

# 3. Hitung Saldo Raw
# Jika Adjusted 10, DO 0 -> 10 - 0 = 10
#reconcile_master['qty_grn_balance_raw'] = (qty_grn_adjusted - reconcile_master['qty_do_comp']).clip(lower=0)
reconcile_master['qty_grn_balance_raw'] = (reconcile_master['qty_grn_base'] - reconcile_master['qty_do_comp']).clip(lower=0)

# 4. CEK POINT PENTING: Flagging Tahun
# Jika is_grn_2026 bernilai 0 atau False, maka 10 * 0 = 0 (Inilah penyebab data "hilang")
reconcile_master['qty_grn_balance'] = reconcile_master['qty_grn_balance_raw'] * reconcile_master['is_grn_2026']

# 5. Hitung Nilai Uang
reconcile_master['amt_grn_balance'] = reconcile_master['qty_grn_balance'] * reconcile_master['net_price_unit'].fillna(0)
