...
# --- RECONCILE LOGIC ENHANCED (DETAIL-ID BASED) ---
...
processed_dfs = []
for df in [df_so_f, df_pr_f, df_po_f, df_po_f, df_si_f]:
    # Filter Customer
    df = df[~df['customer_name'].astype(str).str.upper().isin([c.upper() for c in customers_to_exclude])]
...
df_so_f, df_pr_f, df_po_f, df_po_f, df_si_f = processed_dfs
...
# --- 6. GROUPING DATA (GROUP BY SO_DETAIL_ID) ---
...
pr_base_g = get_grouped(df_pr_base, 'qty_pr_base').rename(
    columns={'transaction_number': 'no_pr_base', 'status_description': 'stat_desc_pr_base', 'description': 'desc_pr'}
)
# pr_comp_g, po_base_g, dst tetap seperti saran sebelumnya yang menggunakan rename unik
...
pr_comp_g = get_grouped(df_pr_comp, 'qty_pr_comp')[['so_detail_id', 'product_id', 'qty_pr_comp', 'transaction_number', 'status_description']].rename(
    columns={'transaction_number': 'no_pr_full', 'status_description': 'stat_desc_pr_full'}
)
po_comp_g = get_grouped(df_po_comp, 'qty_po_comp')[['so_detail_id', 'product_id', 'qty_po_comp', 'transaction_number', 'status_description']].rename(
    columns={'transaction_number': 'no_po_comp', 'status_description': 'stat_desc_po_comp'}
)
grn_comp_g = get_grouped(df_grn_comp, 'qty_grn_comp')[['so_detail_id', 'product_id', 'qty_grn_comp', 'transaction_number', 'status_description']].rename(
    columns={'transaction_number': 'no_grn_comp', 'status_description': 'stat_desc_grn_comp'}
)
do_comp_g = get_grouped(df_do_comp, 'qty_do_comp')[['so_detail_id', 'product_id', 'qty_do_comp', 'transaction_number', 'status_description']].rename(
    columns={'transaction_number': 'no_do_comp', 'status_description': 'stat_desc_do_comp'}
)
si_comp_g = get_grouped(df_si_comp, 'qty_si_comp')[['so_detail_id', 'product_id', 'qty_si_comp', 'transaction_number', 'status_description']].rename(
    columns={'transaction_number': 'no_si_comp', 'status_description': 'stat_desc_si_comp'}
)

# --- 7. MERGE MASTER (REVISED) ---
reconcile_master = so_g.copy()

# Hapus kolom lama yang berpotensi konflik nama
cols_to_drop = ['qty_pr_comp', 'qty_pr_base', 'no_pr', 'stat_desc_pr', 'transaction_number', 'status_description']
reconcile_master = reconcile_master.drop(columns=[c for c in cols_to_drop if c in reconcile_master.columns])

# 1. Merge PR Base (Agar kolom 'qty_pr_base' tersedia untuk kalkulasi PR Balance)
reconcile_master = pd.merge(reconcile_master, pr_base_g, on=['so_detail_id', 'product_id'], how='left')

# 2. Merge PR Comp (Untuk qty_pr_comp = 200 dan nomor dokumen lengkap)
reconcile_master = pd.merge(reconcile_master, pr_comp_g, on=['so_detail_id', 'product_id'], how='left')

# 3. Merge Sisa Dataframes
for other_df in [po_base_g, po_comp_g, grn_base_g, grn_comp_g, do_base_g, do_comp_g, si_comp_g]:
    reconcile_master = pd.merge(reconcile_master, other_df, on=['so_detail_id', 'product_id'], how='left')
...
# --- 11. KALKULASI ---
# Sekarang no_pr akan mengambil dari no_pr_full (yang berisi SIBPR...)
reconcile_master['no_pr'] = reconcile_master['no_pr_full']
reconcile_master['stat_desc_pr'] = reconcile_master['stat_desc_pr_full']
