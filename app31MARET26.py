...
# 2. Gabungkan dengan aman menggunakan re.escape
pattern = '|'.join([re.escape(word) for word in keyword_to_exclude2])

processed_dfs = []
for df in [df_so_f, df_pr_f, df_do_f, df_si_f]:
    # Filter Customer
    df = df[~df['customer_name'].astype(str).str.upper().isin([c.upper() for c in customers_to_exclude])]
    ...
    # Filter Item Type
    if 'description' in df.columns:
        # 3. Gunakan variabel 'pattern' di sini, bukan keyword_to_exclude2
        df = df[~df['item_name'].astype(str).str.contains(pattern, case=False, na=False)]
    
    processed_dfs.append(df)

# Kembalikan ke variabel asli
df_so_f, df_pr_f, df_do_f, df_si_f = processed_dfs
