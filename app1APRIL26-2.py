...
pattern = '|'.join([re.escape(word) for word in keyword_to_exclude2])

# --- TAHAP 1: Filter Customer (untuk SO, PR, DO, SI) ---
# Kita simpan dalam list agar mudah dilooping
dfs_customer = [df_so_f, df_pr_f, df_do_f, df_si_f]
...
df_so_f, df_pr_f, df_do_f, df_si_f = dfs_customer


# --- TAHAP 2: Filter Keyword (untuk SO, PR, PO, DO, SI) ---
# Perhatikan ada tambahan df_po_f di sini
dfs_keyword = [df_so_f, df_pr_f, df_po_f, df_do_f, df_si_f]
processed_keyword_dfs = []

for df in dfs_keyword:
    # Filter Deskripsi (Konsinyasi)
    if 'description' in df.columns:
        df = df[~df['description'].astype(str).str.contains(keyword_to_exclude1, case=False, na=False)]
...
df_so_f, df_pr_f, df_po_f, df_do_f, df_si_f = processed_keyword_dfs
